...
class login(webDriver, unittest.TestCase):
    ...
    def test_Login(self):
        ...
        if self._testMethodDoc == 'test_login1':
            time.sleep(5)
            islogin = self.driver.find_element_by_xpath("//*[@id ='setting']").text
        ...
        #对用户名错误,密码正确的case
            ...
            myLog.logger().info("预期结果的值是:%s", self.excepted)
            #和预期的结果值进行对比
            self.assertEqual(act_text, self.excepted, msg="用户名密码失败用例失败")
